...
`docs/plans/backlog.md` は deferred 候補と「将来やる作業」の集約場所。active plan の `Improvement Candidates` で `deferred` 判定されたものは archive 前に backlog へ転記、新 plan 起草時はここから候補を pick する。詳細運用ルールと 4 段階 (High/Medium/Low/Cancelled) の使い分けは [ADR 0008 Amendment 2026-05-03](decisions/0008-plan-persistence.md#amendment-2026-05-03-pr-θ-backlog-の導入) を参照。
