...
最初のプロンプトは新規プロジェクト用。観察対象の working tree がないため、`<やりたいこと>` だけは user が伝える必要がある。agent は 11-section の Brief を起草し、target の形 (name、`~/ghq/...` への配置、tier、stack、check コマンド) を提案し、ユーザーの確認を経てから初めてファイルを作成する。
